...
del %rc_file%  && move %rc_file_bak% %rc_file% && cd build\windows\runner\Release && Rar.exe a -r quiet-windows-v%1.zip *.* && mc cp quiet-windows-v%1.zip bc/temp/quiet/v%1 && mc cp quiet-windows-v%1.zip bc/temp/quiet/quiet-windows-latest.zip && del windows-v%1.zip && cd ../../../../ && mc cp %file% bc/temp
}
